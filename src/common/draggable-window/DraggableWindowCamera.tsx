...
export const DraggableWindowCamera: FC<DraggableWindowCameraProps> = (
	props
) => {
	const {
		uniqueKey = null,
	...
	const onTouchDownHandler = useCallback(
		(event: TouchEvent) => {
			moveCurrentWindow();
		},
		[moveCurrentWindow]
	...
	const completeDrag = useCallback(() => {
		...
		if (uniqueKey !== null)
			POS_MEMORY.set(uniqueKey, { x: offsetX, y: offsetY });
	...
	useEffect(() => {
		...
		switch (windowPosition) {
			case DraggableWindowPosition.TOP_CENTER:
				...
				element.style.left = `calc(50vw - ${
					element.offsetWidth / 2 + offsetLeft
				}px)`;
				break;
			case DraggableWindowPosition.CENTER:
				element.style.top = `calc(50vh - ${
					element.offsetHeight / 2 + offsetTop
				}px)`;
				element.style.left = `calc(50vw - ${
					element.offsetWidth / 2 + offsetLeft
				}px)`;
	...
	useEffect(() => {
		...
		const element = elementRef.current;
		if (!element) return;
		element.style.transform = `translate(${offset.x + delta.x}px, ${
			offset.y + delta.y
		}px)`;
	...
	useEffect(() => {
		...
		element.addEventListener("touchstart", onTouchDownHandler, {
			passive: true,
		});
	...
	useEffect(() => {
		if (!uniqueKey) return;
		const localStorage = GetLocalStorage<WindowSaveOptions>(
			`nitro.windows.${uniqueKey}`
		);
	...
	return createPortal(
		<Base
			position="absolute"
			innerRef={elementRef}
			className={`draggable-window-camera ${uniqueKey}`}
