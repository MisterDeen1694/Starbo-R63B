...
export const AutoGrid: FC<AutoGridProps> = (props) => {
	...
	const getStyle = useMemo(() => {
		...
		if (columnCount > 1 && grow)
			newStyle.gridTemplateColumns = `repeat(auto-fill, minmax(${columnMinWidth}px, "1fr"))`;
		if (columnCount > 1 && !grow)
			newStyle.gridTemplateColumns = `repeat(auto-fill, minmax(${columnMinWidth}px))`;
