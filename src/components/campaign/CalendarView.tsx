...
export const CalendarView: FC<CalendarViewProps> = (props) => {
	...
	const onClickItem = (item: number) => {
		if (selectedDay === item) {
			...
			if (state === CalendarItemState.STATE_LOCKED_AVAILABLE)
				openPackage(item, false);
		...
		<NitroCardView className="nitro-campaign-calendar" theme="primary-slim">
			<NitroCardHeaderView
				headerText={LocalizeText(
					`campaign.calendar.${campaignName}.title`
				)}
								...
								return (
									<Column key={i} overflow="hidden">
										<CalendarItemView
											...
											product={
												receivedProducts.has(day)
													? receivedProducts.get(day)
													: null
											}
											onClick={onClickItem}
										/>
