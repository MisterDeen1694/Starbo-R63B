...
export const GameView = () => {
	...
	useEffect(() => {
		if (selectedGame) {
			SendMessageComposer(
				new GetGameStatusMessageComposer(selectedGame.gameId)
			);
			SendMessageComposer(
				new Game2GetAccountGameStatusMessageComposer(
					selectedGame.gameId
				)
			);
		}
	...
	return (
		...
		>
			<Flex className="w-75" column alignItems="center" gap={2}>
				<Text bold>
					{LocalizeText(
						`gamecenter.${selectedGame.gameNameId}.description_title`
					)}
				</Text>
				<img
					src={
						selectedGame.assetUrl +
						selectedGame.gameNameId +
						"_logo.png"
					}
				/>
				{(accountStatus.hasUnlimitedGames ||
					accountStatus.freeGamesLeft > 0) && (
					<>
						<Button variant="primary" onClick={onPlay}>
							{LocalizeText("gamecenter.play_now")}
							{!accountStatus.hasUnlimitedGames && (
								<LayoutItemCountView
									className="me-n1 mt-n1 text-black"
									count={accountStatus.freeGamesLeft}
								/>
							)}
						</Button>
					</>
				...
				<Text bold className="w-50" center>
					{LocalizeText(
						`gamecenter.${selectedGame.gameNameId}.description_content`
					)}
				</Text>
			</Flex>
