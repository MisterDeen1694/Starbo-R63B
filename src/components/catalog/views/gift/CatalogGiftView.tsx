...
export const CatalogGiftView: FC<{}> = (props) => {
	...
	const isBoxDefault = useMemo(() => {
		return giftConfiguration
			? giftConfiguration.defaultStuffTypes.findIndex(
					(s) => s === boxTypes[selectedBoxIndex]
			  ) > -1
			: false;
	...
	const boxExtraData = useMemo(() => {
		...
		return (
			boxTypes[selectedBoxIndex] * 1000 +
			giftConfiguration.ribbonTypes[selectedRibbonIndex]
		).toString();
	...
	const allFriends = friends.filter(
		(friend: MessengerFriend) => friend.id !== -1
	);
			...
			switch (action) {
				case "prev_box":
					setSelectedBoxIndex((value) =>
						value === 0 ? maxBoxIndex : value - 1
					);
					return;
				case "next_box":
					setSelectedBoxIndex((value) =>
						value === maxBoxIndex ? 0 : value + 1
					);
					return;
				case "prev_ribbon":
					setSelectedRibbonIndex((value) =>
						value === 0 ? maxRibbonIndex : value - 1
					);
					return;
				case "next_ribbon":
					setSelectedRibbonIndex((value) =>
						value === maxRibbonIndex ? 0 : value + 1
					);
					return;
				case "buy":
	...
	useMessageEvent<GiftReceiverNotFoundEvent>(
		GiftReceiverNotFoundEvent,
		(event) => setReceiverNotFound(true)
	);

	useUiEvent(
		[CatalogPurchasedEvent.PURCHASE_SUCCESS, CatalogEvent.INIT_GIFT],
		(event) => {
			switch (event.type) {
		...
		setBoxTypes((prev) => {
			...
			newPrev.push(
				giftConfiguration.defaultStuffTypes[
					Math.floor(
						Math.random() *
							(giftConfiguration.defaultStuffTypes.length - 1)
					)
				]
			);
	...
	if (!giftConfiguration || !giftConfiguration.isEnabled || !isVisible)
		return null;

	const boxName =
		"catalog.gift_wrapping_new.box." +
		(isBoxDefault ? "default" : boxTypes[selectedBoxIndex]);
	const ribbonName = `catalog.gift_wrapping_new.ribbon.${selectedRibbonIndex}`;
	const priceText =
		"catalog.gift_wrapping_new." + (isBoxDefault ? "freeprice" : "price");
	...
	return (
		...
		>
			<NitroCardHeaderView
				headerText={LocalizeText("catalog.gift_wrapping.title")}
				onCloseClick={onClose}
			/>
			<NitroCardContentView className="text-black">
				<FormGroup column>
					<Text>
						{LocalizeText("catalog.gift_wrapping.receiver")}
					</Text>
					<input
						type="text"
						className={classNames(
							"form-control form-control-sm",
							receiverNotFound && "is-invalid"
						)}
						value={receiverName}
						...
						<Column className="autocomplete-gift-container">
							{suggestions.map((friend: MessengerFriend) => (
								<Base
									...
									onClick={(e) =>
										selectedReceiverName(friend.name)
									}
								>
									{friend.name}
								</Base>
					...
					{receiverNotFound && (
						<Base className="invalid-feedback">
							{LocalizeText(
					...
					<Column gap={1}>
						<Flex gap={2}>
							<ButtonGroup>
								<Button
									variant="primary"
									onClick={() => handleAction("prev_box")}
								>
									<FaChevronLeft className="fa-icon" />
								</Button>
								<Button
									variant="primary"
									onClick={() => handleAction("next_box")}
								>
							...
							<Column gap={1}>
								...
								<Flex alignItems="center" gap={1}>
									{LocalizeText(
										priceText,
										["price"],
										[giftConfiguration.price.toString()]
									)}
						...
						>
							<ButtonGroup>
								<Button
									variant="primary"
									onClick={() => handleAction("prev_ribbon")}
								>
									<FaChevronLeft className="fa-icon" />
								</Button>
								<Button
									variant="primary"
									onClick={() => handleAction("next_ribbon")}
								>
									<FaChevronRight className="fa-icon" />
								</Button>
							</ButtonGroup>
				...
				<Column
					gap={1}
					className={
						isColorable ? "" : "opacity-50 pointer-events-none"
					}
				>
					<Text fontWeight="bold">
					...
					<ButtonGroup fullWidth>
						{colors.map((color) => (
							<Button
								key={color.id}
								variant="primary"
								/*active={ (color.id === selectedColorId) }*/ disabled={
									!isColorable
								}
