...
export const OfferWindowView = (props: {
	offer: TargetedOfferData;
	...
	const canPurchase = useMemo(() => {
		...
		if (offer.priceInCredits > 0)
			credits = getCurrencyAmount(-1) >= offer.priceInCredits;

		if (offer.priceInActivityPoints > 0)
			points =
				getCurrencyAmount(offer.activityPointType) >=
	...
	const expirationTime = () => {
		let expirationTime = Math.max(
			0,
			(offer.expirationTime - Date.now()) / 1000
		);
	...
	const buyOffer = () => {
		SendMessageComposer(
			new PurchaseTargetedOfferComposer(offer.id, amount)
		);
	...
	return (
		...
		>
			<NitroCardHeaderView
				headerText={LocalizeText(offer.title)}
				onCloseClick={(event) => setOpen(false)}
			/>
			<NitroCardSubHeaderView
				position="relative"
			...
			<NitroCardContentView gap={1}>
				<Flex gap={1} fullHeight>
					...
					<Base
						...
						style={{
							background: `url(${
								GetConfiguration("image.library.url") +
								offer.imageUrl
							}) no-repeat center`,
						}}
					/>
