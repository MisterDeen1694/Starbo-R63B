...
export const CatalogSpacesWidgetView: FC<CatalogSpacesWidgetViewProps> = (
	props
) => {
	const { columnCount = 5, children = null, ...rest } = props;
	const [groupedOffers, setGroupedOffers] =
		useState<IPurchasableOffer[][]>(null);
	const [selectedGroupIndex, setSelectedGroupIndex] = useState(-1);
	const [selectedOfferForGroup, setSelectedOfferForGroup] =
		useState<IPurchasableOffer[]>(null);
	const {
		currentPage = null,
		currentOffer = null,
	...
	useEffect(() => {
		...
		for (const offer of currentPage.offers) {
			if (
				offer.pricingModel !== Offer.PRICING_MODEL_SINGLE &&
			...
			if (
				!product ||
				(product.productType !== ProductTypeEnum.WALL &&
	...
	useEffect(() => {
		...
		setPurchaseOptions((prevValue) => {
			...
			newValue.extraData =
				selectedOfferForGroup[selectedGroupIndex].product.extraParam;
		...
		<>
			<ButtonGroup>
				{SPACES_GROUP_NAMES.map((name, index) => (
					<Button
						key={index}
						/*active={ (selectedGroupIndex === index) }*/ onClick={(
							event
