...
export const CatalogLayoutColorGroupingView: FC<
	CatalogLayoutColorGroupViewProps
> = (props) => {
	const { page = null } = props;
	const [colorableItems, setColorableItems] = useState<Map<string, number[]>>(
	...
	const sortByColorIndex = (a: IPurchasableOffer, b: IPurchasableOffer) => {
		if (
			!a.product.furnitureData.colorIndex ||
			!b.product.furnitureData.colorIndex
		) {
			return 1;
		}
		if (
	...
	const sortyByFurnitureClassName = (
		a: IPurchasableOffer,
		b: IPurchasableOffer
	) => {
		if (
			a.product.furnitureData.className >
			b.product.furnitureData.className
		) {
			return 1;
		}
		if (a == b) {
	...
	const selectColor = (colorIndex: number, productName: string) => {
		...
		const index = page.offers.findIndex(
			(offer) => offer.product.furnitureData.fullName === fullName
		);
		if (index > -1) {
		...
		page.offers.forEach((offer) => {
			...
			if (!furniData || !furniData.hasIndexedColor) {
				...
				if (furniData.colors) {
					...
					if (
						updatedColorableItems
							.get(name)
							.indexOf(selectedColor) === -1
					) {
						updatedColorableItems.get(name)[colorIndex] =
							selectedColor;
					}
				}
					...
					{(!colorsShowing ||
						...
						offers.map((offer, index) => (
							<CatalogGridOfferView
								...
								itemActive={
									currentOffer &&
									(currentOffer.product.furnitureData
										.hasIndexedColor
										? currentOffer.product.furnitureData
												.className ===
				...
				{!currentOffer && (
					<>
						{!!page.localization.getImage(1) && (
							<img alt="" src={page.localization.getImage(1)} />
						)}
						<Text
							center
							dangerouslySetInnerHTML={{
								__html: page.localization.getText(0),
							}}
						/>
					</>
				)}
