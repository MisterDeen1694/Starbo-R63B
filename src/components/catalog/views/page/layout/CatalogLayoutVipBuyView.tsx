...
export const CatalogLayoutVipBuyView: FC<CatalogLayoutProps> = (props) => {
	...
	const getOfferText = useCallback((offer: ClubOfferData) => {
		...
		if (offer.months > 0) {
			offerText = LocalizeText(
				"catalog.vip.item.header.months",
				["num_months"],
				[offer.months.toString()]
			);
		}
	...
	const getPurchaseHeader = useCallback(() => {
		...
		const extensionOrSubscription =
			purse.clubDays > 0 || purse.clubPeriods > 0
				? "extension."
				: "subscription.";
		const daysOrMonths = pendingOffer.months === 0 ? "days" : "months";
		const daysOrMonthsText =
			pendingOffer.months === 0
				? pendingOffer.extraDays
				: pendingOffer.months;
		const locale = LocalizeText(
			"catalog.vip.buy.confirm." + extensionOrSubscription + daysOrMonths
		);

		return locale.replace(
			"%NUM_" + daysOrMonths.toUpperCase() + "%",
			daysOrMonthsText.toString()
		);
	}, [pendingOffer, purse]);
	...
	const getPurchaseButton = useCallback(() => {
		...
		if (
			...
			getCurrencyAmount(pendingOffer.priceActivityPointsType)
		) {
			return (
				<Button variant="primary">
					{LocalizeText(
						"catalog.alert.notenough.activitypoints.title." +
							pendingOffer.priceActivityPointsType
					)}
				</Button>
			);
		}
		...
		switch (purchaseState) {
			case CatalogPurchaseState.CONFIRM:
				return (
					<Button variant="primary" onClick={purchaseSubscription}>
	...
	useEffect(() => {
		if (!clubOffers)
			SendMessageComposer(new GetClubOffersMessageComposer(1));
	}, [clubOffers]);
			...
			>
				...
				<Text
					...
					dangerouslySetInnerHTML={{
						__html: LocalizeText("catalog.vip.buy.hccenter"),
					}}
				></Text>
			</Column>
			<Column size={5} overflow="hidden">
				<Column fullHeight center overflow="hidden">
					{currentPage.localization.getImage(1) && (
						<img
							alt=""
							src={currentPage.localization.getImage(1)}
						/>
					)}
					<Text
						center
