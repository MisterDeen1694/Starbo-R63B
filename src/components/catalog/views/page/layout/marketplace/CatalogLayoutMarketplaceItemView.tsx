...
export const CatalogLayoutMarketplaceItemView: FC<MarketplaceItemViewProps> = (
	props
) => {
	...
	const getMarketplaceOfferTitle = useMemo(() => {
		...
		// desc
		return LocalizeText(
			(offerData.furniType === 2 ? "wallItem" : "roomItem") +
				`.name.${offerData.furniId}`
		);
	}, [offerData]);
	...
	const offerTime = useCallback(() => {
		...
		if (offerData.status === MarketPlaceOfferState.SOLD)
			return LocalizeText("catalog.marketplace.offer.sold");

		if (offerData.timeLeftMinutes <= 0)
			return LocalizeText("catalog.marketplace.offer.expired");
		...
		let text =
			minutes + " " + LocalizeText("catalog.marketplace.offer.minutes");
		if (hours > 0) {
			text =
				hours +
				" " +
				LocalizeText("catalog.marketplace.offer.hours") +
				" " +
				text;
		}

		return LocalizeText(
			"catalog.marketplace.offer.time_left",
			["time"],
			[text]
		);
	}, [offerData]);
	...
	return (
		...
		>
				...
				<LayoutGridItem
					...
					itemUniqueNumber={
						offerData.isUniqueLimitedItem
							? offerData.stuffData.uniqueNumber
							: 0
					}
				/>
			</Column>
							...
							{LocalizeText(
								...
								[
									...
									offerData.averagePrice > 0
										? offerData.averagePrice.toString()
										: "-",
								]
							)}
						</Text>
			...
			<Column gap={1}>
				{type === OWN_OFFER &&
					offerData.status !== MarketPlaceOfferState.SOLD && (
						<Button
							variant="primary"
