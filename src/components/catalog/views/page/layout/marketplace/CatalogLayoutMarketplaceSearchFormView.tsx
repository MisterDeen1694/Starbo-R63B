...
export const SearchFormView: FC<SearchFormViewProps> = (props) => {
	...
	const onSortTypeChange = useCallback(
		(sortType: number) => {
			...
			if (
				searchType === MarketplaceSearchType.BY_ACTIVITY ||
				searchType === MarketplaceSearchType.BY_VALUE
			)
				onSearch({
					minPrice: -1,
					maxPrice: -1,
	...
	useEffect(() => {
		...
		if (
			searchType === MarketplaceSearchType.BY_ACTIVITY ||
			MarketplaceSearchType.BY_VALUE === searchType
		)
		...
		<Column gap={1}>
			<Flex alignItems="center" gap={1}>
				...
				<select
					...
					value={sortType}
					onChange={(event) =>
						onSortTypeChange(parseInt(event.target.value))
					}
				>
					{sortTypes.map((type) => (
