...
import {
	...
	PUBLIC_OFFER,
} from "./CatalogLayoutMarketplaceItemView";
...
export interface CatalogLayoutMarketplacePublicItemsViewProps
	extends CatalogLayoutProps {}

export const CatalogLayoutMarketplacePublicItemsView: FC<
	CatalogLayoutMarketplacePublicItemsViewProps
> = (props) => {
	const [searchType, setSearchType] = useState(
		MarketplaceSearchType.BY_ACTIVITY
	);
	const [totalItemsFound, setTotalItemsFound] = useState(0);
	const [offers, setOffers] = useState(
		new Map<number, MarketplaceOfferData>()
	);
	...
	const requestOffers = useCallback((options: IMarketplaceSearchOptions) => {
		...
		SendMessageComposer(
			new GetMarketplaceOffersMessageComposer(
				options.minPrice,
				options.maxPrice,
				options.query,
				options.type
	...
	const purchaseItem = useCallback(
		(offerData: MarketplaceOfferData) => {
			...
			showConfirm(
				...
				() => {
					SendMessageComposer(
						new BuyMarketplaceOfferMessageComposer(offerId)
					);
				},
				null,
				null,
	...
	useMessageEvent<MarketplaceBuyOfferResultEvent>(
		MarketplaceBuyOfferResultEvent,
		(event) => {
			const parser = event.getParser();
			...
			switch (parser.result) {
				...
				case 3:
					...
					showConfirm(
						LocalizeText(
							"catalog.marketplace.confirm_higher_header"
						) +
							"\n" +
							LocalizeText(
								"catalog.marketplace.confirm_price",
								["price"],
								[parser.newPrice.toString()]
							),
						() => {
							SendMessageComposer(
								new BuyMarketplaceOfferMessageComposer(
									parser.offerId
								)
							);
						},
	...
	return (
		<>
			<ButtonGroup>
				<Button
					/*active={ (searchType === MarketplaceSearchType.BY_ACTIVITY) }*/ onClick={() =>
						setSearchType(MarketplaceSearchType.BY_ACTIVITY)
					}
				>
					{LocalizeText("catalog.marketplace.search_by_activity")}
				</Button>
				<Button
					/*active={ (searchType === MarketplaceSearchType.BY_VALUE) } */ onClick={() =>
						setSearchType(MarketplaceSearchType.BY_VALUE)
					}
				>
					{LocalizeText("catalog.marketplace.search_by_value")}
				</Button>
				<Button
					/*active={ (searchType === MarketplaceSearchType.ADVANCED) } */ onClick={() =>
						setSearchType(MarketplaceSearchType.ADVANCED)
					}
				>
			...
			<SearchFormView
				...
				onSearch={requestOffers}
			/>
			<Column gap={1} overflow="hidden">
				<Text truncate shrink fontWeight="bold">
					{LocalizeText(
						"catalog.marketplace.items_found",
						["count"],
						[offers.size.toString()]
					)}
				</Text>
				<Column
