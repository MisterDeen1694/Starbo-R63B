...
export const CatalogLayoutMarketplaceOwnItemsView: FC<CatalogLayoutProps> = (
	props
) => {
	...
	useMessageEvent<MarketplaceOwnOffersEvent>(
		MarketplaceOwnOffersEvent,
		(event) => {
	...
	useMessageEvent<MarketplaceCancelOfferResultEvent>(
		MarketplaceCancelOfferResultEvent,
		(event) => {
			const parser = event.getParser();
			...
			setOffers((prevValue) =>
				prevValue.filter((value) => value.offerId !== parser.offerId)
			);
		}
	);
	...
	const soldOffers = useMemo(() => {
		return offers.filter(
			(value) => value.status === MarketPlaceOfferState.SOLD
		);
	}, [offers]);
	...
	const redeemSoldOffers = useCallback(() => {
		setOffers((prevValue) => {
			...
			return prevValue.filter(
				(value) => idsToDelete.indexOf(value.offerId) === -1
			);
		});
	...
	const takeItemBack = (offerData: MarketplaceOfferData) => {
		SendMessageComposer(
			new CancelMarketplaceOfferMessageComposer(offerData.offerId)
		);
	};
				...
				<Column center gap={1} className="bg-muted rounded p-2">
					<Text>
						{LocalizeText(
							...
							["count", "credits"],
							[
								soldOffers.length.toString(),
								creditsWaiting.toString(),
							]
						)}
					</Text>
					<Button onClick={redeemSoldOffers}>
