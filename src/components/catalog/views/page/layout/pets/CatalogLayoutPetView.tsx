...
export const CatalogLayoutPetView: FC<CatalogLayoutProps> = (props) => {
	...
	const [sellablePalettes, setSellablePalettes] = useState<
		SellablePetPaletteData[]
	>([]);
	...
	const getColor = useMemo(() => {
		if (!sellableColors.length || selectedColorIndex === -1)
			return 0xffffff;
	...
	const petBreedName = useMemo(() => {
		if (
			petIndex === -1 ||
			!sellablePalettes.length ||
			selectedPaletteIndex === -1
		)
			return "";

		return LocalizeText(
			`pet.breed.${petIndex}.${sellablePalettes[selectedPaletteIndex].breedId}`
		);
	...
	useMessageEvent<ApproveNameMessageEvent>(
		ApproveNameMessageEvent,
		(event) => {
	...
	useEffect(() => {
		...
		SendMessageComposer(
			new GetSellablePetPalettesComposer(productData.type)
		);
					...
					<>
						...
						<Column grow gap={1}>
							...
							<Column grow gap={1}>
								...
								/>
								{approvalResult > 0 && (
									<Base className="invalid-feedback d-block m-0">
