...
export const GuideToolAcceptView: FC<GuideToolAcceptViewProps> = (props) => {
	...
	const answerRequest = (response: boolean) =>
		SendMessageComposer(
			new GuideSessionGuideDecidesMessageComposer(response)
		);
	...
	return (
		<Column>
			<Column gap={0} className="bg-muted p-2 rounded">
				<Text bold>
					{LocalizeText(
						"guide.help.request.guide.accept.request.title"
					)}
				</Text>
				<Text variant="muted">
			...
			<Column gap={1}>
				<Button
					variant="primary"
					onClick={(event) => answerRequest(true)}
				>
					{LocalizeText(
						"guide.help.request.guide.accept.accept.button"
					)}
				</Button>
				<Button
					variant="primary"
					onClick={(event) => answerRequest(false)}
				>
