...
export const GuideToolOngoingView: FC<GuideToolOngoingViewProps> = (props) => {
	...
	useMessageEvent<GuideSessionRequesterRoomMessageEvent>(
		GuideSessionRequesterRoomMessageEvent,
		(event) => {
	...
	const sendMessage = useCallback(() => {
		...
		SendMessageComposer(
			new GuideSessionMessageMessageComposer(messageText)
		);
		setMessageText("");
	}, [messageText]);
			...
			>
				{isGuide && (
					<ButtonGroup>
						<Button onClick={visit}>
							{LocalizeText(
								"guide.help.request.guide.ongoing.visit.button"
							)}
						</Button>
						<Button onClick={invite}>
							{LocalizeText(
								"guide.help.request.guide.ongoing.invite.button"
							)}
						</Button>
					</ButtonGroup>
				)}
				{!isGuide && (
					<Column gap={0}>
						...
						<Text>
							{LocalizeText(
								"guide.help.request.user.ongoing.guide.desc"
							)}
						</Text>
					</Column>
				)}
					...
					{messageGroups.map((group, index) => {
						return (
							<Flex
								...
								fullWidth
								justifyContent={
									isOwnChat(group.userId) ? "end" : "start"
								}
								gap={2}
							>
								...
								<Base
									className={
										"bg-light text-black border-radius mb-2 rounded py-1 px-2 messages-group-" +
										(isOwnChat(group.userId)
											? "right"
											: "left")
									}
								>
									...
									{group.messages.map((chat, index) => (
										<Base
											...
											className={
												chat.roomId
													? "text-break text-underline"
													: "text-break"
											}
											onClick={() =>
												chat.roomId
													? TryVisitRoom(chat.roomId)
													: null
											}
										>
											{chat.message}
