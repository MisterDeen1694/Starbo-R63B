...
export const WiredBaseView: FC<PropsWithChildren<WiredBaseViewProps>> = (
	props
) => {
	const {
		wiredType = "",
	...
	const {
		...
		setAllowsFurni,
		saveWired,
		setMaxItemSelectionCount: setMaxCount,
	} = useWired();
	...
	useEffect(() => {
		...
		if (requiresFurni > WiredFurniType.STUFF_SELECTION_OPTION_NONE) {
			setFurniIds((prevValue) => {
				...
				if (trigger.selectedItems && trigger.selectedItems.length) {
					WiredSelectionVisualizer.applySelectionShaderToFurni(
		...
		if (WiredSelectionVisualizer.setMaxItemSelectionCount) {
			WiredSelectionVisualizer.setMaxItemSelectionCount(
				maxItemSelectionCount
			);
		}
	}, [
		trigger,
