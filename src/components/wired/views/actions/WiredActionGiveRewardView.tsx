...
export const WiredActionGiveRewardView: FC<{}> = (props) => {
	...
	const [rewards, setRewards] = useState<
		{ isBadge: boolean; itemCode: string; probability: number }[]
	>([]);
	const {
		trigger = null,
		setIntParams = null,
	...
	const addReward = () =>
		setRewards((rewards) => [
			...rewards,
			{ isBadge: false, itemCode: "", probability: null },
		]);
	...
	useEffect(() => {
		...
		setUniqueRewards(
			trigger.intData.length > 1 ? trigger.intData[1] === 1 : false
		);
		setRewardsLimit(trigger.intData.length > 2 ? trigger.intData[2] : 0);
		setLimitationInterval(
			trigger.intData.length > 3 ? trigger.intData[3] : 0
		);
		setLimitEnabled(
			trigger.intData.length > 3 ? trigger.intData[3] > 0 : false
		);
		...
		>
			...
			{!limitEnabled && (
				<Text center small className="bg-muted rounded p-1">
					Reward limit not set. Make sure rewards are badges or
					non-tradeable items.
				</Text>
			)}
			{limitEnabled && (
			...
			<Column gap={1}>
				...
				<Flex gap={1}>
					...
					>
						...
						<option value="3">
							Once every {limitationInterval} minutes
						</option>
						<option value="2">
							Once every {limitationInterval} hours
						</option>
						<option value="1">
							Once every {limitationInterval} days
						</option>
					</select>
					{rewardTime > 0 && (
						<input
			...
			<Text center small className="bg-muted rounded p-1">
				If checked each reward will be given once to each user. This
				will disable the probabilities option.
			</Text>
			<hr className="m-0 bg-dark" />
