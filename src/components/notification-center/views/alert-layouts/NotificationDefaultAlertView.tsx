...
export const NotificationDefaultAlertView: FC<
	NotificationDefaultAlertViewProps
> = (props) => {
	const {
		item = null,
	...
	const options = (
		<Column alignItems="center" center gap={0} className="my-1">
			{!item.clickUrl && (
	...
	return (
		...
		>
			<Flex
				fullHeight
				overflow="auto"
				gap={hasFrank || (item.imageUrl && !imageFailed) ? 2 : 0}
			>
				{hasFrank && !item.imageUrl && (
					<Base className="notification-frank flex-shrink-0" />
				)}
				...
				<Base
					classNames={[
						"notification-text overflow-y-auto d-flex flex-column w-100",
						item.clickUrl && !hasFrank
