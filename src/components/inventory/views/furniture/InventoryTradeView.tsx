...
export const InventoryTradeView: FC<InventoryTradeViewProps> = (props) => {
			...
			.map(
				(item) =>
					Number(
						item.iconUrl
							.split("/")
							[item.iconUrl.split("/").length - 1]?.split("_")[1]
					) * item.items.length
			)
			.reduce((acc, cur) => acc + (isNaN(cur) ? 0 : cur), 0);
	};
	...
	if (tradeState === TradeState.TRADING_STATE_READY || !ownUser || !otherUser)
		return null;
	...
	return (
		<Column>
			<Column
				...
				size={12}
				overflow="hidden"
				className="trade-bg p-2 text-black"
			>
				{currentTab === TAB_FURNITURE && (
					<>
						<Text small>
							{LocalizeText("inventory.trading.info.add")}
						</Text>
						<Grid gap={0} overflow="hidden" className="px-2">
							<Column size={4} overflow="hidden">
								<Flex>
									{ownUser.accepts && (
										<Base className="icon icon-confirmed" />
									)}
									<small
										className={
											ownUser.accepts ? "px-2" : ""
										}
									>
										<b>
											{LocalizeText(
												"inventory.trading.you"
											)}
										</b>{" "}
										{LocalizeText(
								...
								>
									{Array.from(
										Array(MAX_ITEMS_TO_TRADE),
										(e, i) => {
											const item =
												ownUser.userItems.getWithIndex(
													i
												) || null;
								...
								<Column gap={0} fullWidth>
									<small>
										{LocalizeText(
											...
											["value"],
											[ownUser.itemCount.toString()]
										)}
									</small>
									<small className="credits-align">
										{LocalizeText(
											"inventory.trading.info.creditvalue.own",
											["value"],
											[
												getTotalCredits(
													ownUser.userItems
							...
							<Column size={4} overflow="hidden">
								<Flex>
									...
									<small
										className={
											otherUser.accepts ? "px-2" : ""
										}
									>
										<b>{otherUser.userName}</b>{" "}
										{LocalizeText(
											"inventory.trading.isoffering"
										)}
									</small>
								</Flex>
								<AutoGrid
								...
								>
									{Array.from(
										...
										(e, i) => {
											const item =
												otherUser.userItems.getWithIndex(
													i
												) || null;
				...
				{currentTab !== TAB_FURNITURE && (
					<>
						<Flex className="px-2">
							...
							<Text bold>
								{LocalizeText(
									"inventory.trading.minimized.trade_in_progress"
								)}
							</Text>
						</Flex>
					</>
				...
				<Flex grow justifyContent="between">
					{currentTab === TAB_FURNITURE && (
						<>
							...
							{tradeState ===
								TradeState.TRADING_STATE_RUNNING && (
								<Button
									variant="primary"
									disabled={
										!ownUser.itemCount &&
							...
							{tradeState ===
								TradeState.TRADING_STATE_COUNTDOWN && (
								<Button variant="primary" disabled>
									{LocalizeText(
										"inventory.trading.countdown",
										["counter"],
										[countdownTick.toString()]
									)}
								</Button>
							)}
							{tradeState ===
								TradeState.TRADING_STATE_CONFIRMING && (
								<Button
									variant="primary"
									onClick={progressTrade}
								>
									{LocalizeText(
										"inventory.trading.button.restore"
									)}
								</Button>
							)}
							{tradeState ===
								TradeState.TRADING_STATE_CONFIRMED && (
					...
					{currentTab !== TAB_FURNITURE && (
						...
						>
							{LocalizeText(
								"inventory.trading.minimized.continue_trade"
							)}
						</Button>
					)}
					<Button variant="primary" onClick={cancelTrade}>
