import {
	DeleteFurniTypeInventoryComposer,
	IObjectData,
	IRoomSession,
	RoomObjectVariable,
	RoomPreviewer,
...
import {
	CatalogPostMarketplaceOfferEvent,
...
const attemptDeleteItem = (groupItem: GroupItem) => {
	...
	DispatchUiEvent(
		new DeleteItemConfirmEvent(item, groupItem.getTotalCount())
	);
};

export const InventoryFurnitureView: FC<InventoryFurnitureViewProps> = (
	props
) => {
	const {
		roomSession = null,
	...
	const attemptItemOffer = (count: number) => {
		...
		if (ownItemCount + itemIds.length <= 1500) {
			if (!coreItem.isGroupable && itemIds.length) {
				SendMessageComposer(
					new TradingListAddItemComposer(itemIds.pop())
				);
			} else {
				...
				if (tradeIds.length) {
					if (tradeIds.length === 1) {
						SendMessageComposer(
							new TradingListAddItemComposer(tradeIds.pop())
						);
					} else {
						SendMessageComposer(
							new TradingListAddItemsComposer(...tradeIds)
						);
					}
				}
			}
	...
	useEffect(() => {
		...
		if (
			...
			if (furnitureItem.category === FurniCategory.LANDSCAPE) {
				const data = GetSessionDataManager().getWallItemDataByName(
	...
	const totalItems = !isTrading
		? selectedItem.items.length
		: selectedItem.getUnlockedCount();
	...
	return (
		<Grid className="mt-n1">
			<Column
				...
				overflow="hidden"
				style={{
					height: `calc(100% - ${!isTrading ? "34px" : "5px"})`,
				}}
			>
				<AutoGrid
					gap={1}
					columnCount={5}
					className={isTrading ? "trading-inventory" : ""}
				>
					{filteredGroupItems &&
						...
						filteredGroupItems.map((item, index) => (
							<InventoryFurnitureItemView
								...
								groupItem={item}
								isTrading={isTrading}
								attemptItemOffer={(e) => attemptItemOffer(e)}
								setGroupItem={(e) => setGroupItem(e)}
							/>
						))}
				</AutoGrid>
			</Column>
			<Column size={5} overflow="auto">
				...
				>
					...
					{selectedItem && (
						<Button
							variant="primary"
							onClick={(event) => attemptDeleteItem(selectedItem)}
						>
							<FaTrashAlt className="fa-icon" />
						</Button>
					)}
					{selectedItem &&
						(selectedItem.items[0].isTradable ||
							!selectedItem.items[0].isTradable) && (
							<Flex
								gap={2}
								position="absolute"
							...
							>
								<Base
									...
									title={LocalizeText(
										selectedItem.items[0].isTradable &&
											totalItems > 0
											? "inventory.furni.preview.tradeable_amount"
											: "inventory.furni.preview.not_tradeable"
									)}
								/>
								{selectedItem.items[0].isTradable &&
									totalItems > 0 && (
										<Text
											variant="black"
											className="text-shadow-around-text mt-n1"
										>
											{totalItems}
										</Text>
									)}
					...
					{selectedItem &&
						(selectedItem.items[0].recyclable ||
							!selectedItem.items[0].recyclable) && (
							<Flex
								gap={2}
								position="absolute"
								className="top-4 start-2"
							>
								<Base
									className={`icon ${
										selectedItem.items[0].recyclable &&
									...
									title={LocalizeText(
										selectedItem.items[0].recyclable &&
											totalItems > 0
											? "inventory.furni.preview.recyclable_amount"
											: "inventory.furni.preview.not_recyclable"
									)}
								/>
								{selectedItem.items[0].recyclable &&
									totalItems > 0 && (
										<Text
											variant="black"
											className="text-shadow-around-text"
										>
											{totalItems}
										</Text>
									)}
				...
				{selectedItem && (
					<Column grow justifyContent="between" gap={2}>
						<Column
							gap={1}
							position={!isTrading ? "absolute" : "relative"}
							className="bottom-1"
							style={{ width: !isTrading ? "39%" : "" }}
						>
							<Text grow truncate bold>
								{selectedItem.name}
							</Text>
							{selectedItem.description && (
								<Text grow truncate small>
									{selectedItem.description}
								</Text>
							)}
							{!isTrading && (
							...
							{isTrading && (
								<Column gap={1} alignItems="start">
									<input
										type="number"
										className="quantity-input remove-outline form-control"
