...
export const ModToolsUserModActionView: FC<ModToolsUserModActionViewProps> = (
	props
) => {
	...
	const sendDefaultSanction = () => {
		...
		const messageOrDefault =
			message.trim().length === 0
				? LocalizeText(`help.cfh.topic.${category.id}`)
				: message;

		SendMessageComposer(
			new DefaultSanctionMessageComposer(
				user.userId,
				selectedTopic,
				messageOrDefault
			)
	...
	const sendSanction = () => {
		...
		if (selectedTopic === -1 || selectedAction === -1)
			errorMessage = "You must select a CFH topic and Sanction";
		...
		switch (sanction.actionType) {
			...
			case ModActionDefinition.TRADE_LOCK: {
				...
				SendMessageComposer(
					new ModTradingLockMessageComposer(
						user.userId,
						messageOrDefault,
						numSeconds,
						category.id
	...
	return (
		...
		>
			<NitroCardHeaderView
				headerText={"Mod Action: " + (user ? user.username : "")}
			...
			<NitroCardContentView className="text-black">
				...
				<select
					...
					onChange={(event) =>
						setSelectedAction(parseInt(event.target.value))
					}
				>
					<option value={-1} disabled>
						Sanction Type
				...
				<Flex justifyContent="between" gap={1}>
					...
					<Button variant="primary" onClick={sendSanction}>
						Sanction
					</Button>
				</Flex>
			</NitroCardContentView>
