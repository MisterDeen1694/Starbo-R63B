...
export const ModToolsIssueInfoView: FC<IssueInfoViewProps> = (props) => {
	...
	const closeIssue = (resolutionType: number) => {
		SendMessageComposer(
			new CloseIssuesMessageComposer([issueId], resolutionType)
		);
	...
	return (
		<>
			...
			>
				<NitroCardHeaderView
					headerText={"Resolving issue " + issueId}
					...
					<Grid overflow="auto">
						...
						<Column size={4} gap={1}>
							<Button
								variant="primary"
								onClick={() =>
									setcfhChatlogOpen(!cfhChatlogOpen)
								}
							>
								Chatlog
							</Button>
							<Button
								onClick={(event) =>
									closeIssue(
										CloseIssuesMessageComposer.RESOLUTION_USELESS
									)
								}
							>
								Close as useless
							...
							<Button
								...
								onClick={(event) =>
									closeIssue(
										CloseIssuesMessageComposer.RESOLUTION_ABUSIVE
									)
								}
							>
								Close as abusive
							...
							<Button
								variant="primary"
								onClick={(event) => releaseIssue(issueId)}
							>
								Release
							</Button>
						</Column>
			...
			{cfhChatlogOpen && (
				<CfhChatlogView
					issueId={issueId}
					onCloseClick={() => setcfhChatlogOpen(false)}
				/>
			)}
		</>
	);
