...
export const ModToolsRoomView: FC<ModToolsRoomViewProps> = (props) => {
	...
	const handleClick = (action: string, value?: string) => {
		...
		switch (action) {
			case "alert_only":
				...
				SendMessageComposer(
					new ModeratorActionMessageComposer(
	...
	return (
		...
		>
			<NitroCardHeaderView
				headerText={"Room Info" + (name ? ": " + name : "")}
			...
			<NitroCardContentView className="text-black">
				<Flex gap={2}>
					...
					<Column gap={1}>
						<Button onClick={(event) => TryVisitRoom(roomId)}>
							Visit Room
						</Button>
						<Button
							onClick={(event) =>
								CreateLinkEvent(
									`mod-tools/open-room-chatlog/${roomId}`
								)
							}
						>
							Chatlog
				...
				<Flex justifyContent="between">
					...
					<Button onClick={(event) => handleClick("alert_only")}>
						Send Alert only
					</Button>
				</Flex>
			</NitroCardContentView>
