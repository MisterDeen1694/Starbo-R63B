...
export const GroupRoomInformationView: FC<{}> = (props) => {
	...
	const [groupInformation, setGroupInformation] =
		useState<GroupInformationParser>(null);
	...
	useMessageEvent<HabboGroupDeactivatedMessageEvent>(
		HabboGroupDeactivatedMessageEvent,
		(event) => {
			const parser = event.getParser();
			...
			if (
				!groupInformation ||
				(parser.groupId !== groupInformation.id &&
					parser.groupId !== expectedGroupId)
			)
				return;
	...
	const isRealOwner =
		groupInformation &&
		groupInformation.ownerName === GetSessionDataManager().userName;
	...
	const getButtonText = () => {
		...
		if (
			groupInformation.membershipType ===
				GroupMembershipType.NOT_MEMBER &&
		...
		if (
			groupInformation.membershipType ===
			GroupMembershipType.REQUEST_PENDING
		)
