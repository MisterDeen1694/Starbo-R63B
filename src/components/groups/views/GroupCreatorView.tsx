...
export const GroupCreatorView: FC<GroupCreatorViewProps> = (props) => {
	...
	const [closeAction, setCloseAction] = useState<{ action: () => boolean }>(
		null
	);
	...
	return (
		<NitroCardView className="nitro-group-creator" theme="primary-slim">
			<NitroCardHeaderView
				headerText={LocalizeText("group.create.title")}
				onCloseClick={onCloseClose}
			/>
			<NitroCardContentView>
				...
				<Column overflow="hidden">
					<Flex alignItems="center" gap={2}>
						<Base
							className={`nitro-group-tab-image tab-${currentTab}`}
						/>
						<Column grow gap={0}>
							<Text bold fontSize={4}>
								{LocalizeText(
									`group.create.stepcaption.${currentTab}`
								)}
							</Text>
							<Text>
								{LocalizeText(
									`group.create.stepdesc.${currentTab}`
								)}
							</Text>
						</Column>
					</Flex>
