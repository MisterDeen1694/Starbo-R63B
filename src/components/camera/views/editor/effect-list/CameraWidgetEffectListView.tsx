...
export const CameraWidgetEffectListView: FC<CameraWidgetEffectListViewProps> = (
	props
) => {
	const {
		myLevel = 0,
		...
		<>
			{effects &&
				...
				effects.map((effect, index) => {
					const thumbnailUrl = thumbnails.find(
						(thumbnail) => thumbnail.effectName === effect.name
					);
					const isActive =
						selectedEffects.findIndex(
							(selectedEffect) =>
								selectedEffect.effect.name === effect.name
						) > -1;
					...
					return (
						<CameraWidgetEffectListItemView
							...
							removeEffect={() =>
								processAction("remove_effect", effect.name)
							}
						/>
					);
				})}
