...
export const NavigatorRoomSettingsRightsTabView: FC<
	NavigatorRoomSettingsTabViewProps
> = (props) => {
	const { roomData = null } = props;
	const [usersWithRights, setUsersWithRights] = useState<Map<number, string>>(
		new Map()
	);
	...
	const friendWitoutRights = allFriends.filter(
		(friend) => !usersWithRights.has(friend.id)
	);
	...
	useMessageEvent<FlatControllerAddedEvent>(
		FlatControllerAddedEvent,
		(event) => {
			const parser = event.getParser();
	...
	useMessageEvent<FlatControllerRemovedEvent>(
		FlatControllerRemovedEvent,
		(event) => {
			const parser = event.getParser();
	...
	return (
		<Grid>
			<Column size={6}>
				...
				<Flex
					overflow="hidden"
					className="p-2 bg-white rounded list-container"
				>
					<Column fullWidth overflow="auto" gap={1}>
						{Array.from(usersWithRights.entries()).map(
							([id, name], index) => {
								return (
									<Flex
										key={index}
				...
				<Button
					...
					onClick={(event) =>
						SendMessageComposer(
							new RemoveAllRightsMessageComposer(roomData.roomId)
						)
					}
				>
			...
			<Column size={6}>
				<Text bold>
					{LocalizeText(
						...
						["displayed", "total"],
						[
							friendWitoutRights.length.toString(),
