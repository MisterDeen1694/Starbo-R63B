...
export const NavigatorRoomCreatorView: FC<{}> = (props) => {
	...
	const getRoomModelImage = (name: string) =>
		GetConfiguration<string>("images.url") +
		`/navigator/models/model_${name}.png`;

	const selectModel = (model: IRoomModel, index: number) => {
		if (!model) return;

		if (GetClubMemberLevel() < model.clubLevel)
	...
	return (
		<NitroCardView className="nitro-room-creator" theme="primary">
			<NitroCardHeaderView
				...
				onCloseClick={(event) =>
					CreateLinkEvent("navigator/close-creator")
				}
			/>
			<NitroCardContentView>
					...
					<Grid overflow="hidden">
						...
						>
							<Column gap={1}>
								<Text>
									{LocalizeText(
										"navigator.createroom.roomnameinfo"
									)}
								</Text>
								<input
									type="text"
							...
							<Column grow gap={1}>
								<Text>
									{LocalizeText(
										"navigator.createroom.roomdescinfo"
									)}
								</Text>
								<textarea
											...
											>
												<Base
													className={`icon ${
														selectedModelName ===
														model.name
															? "icon-tiles_room_selected"
															: "icon-tiles"
													}`}
												/>{" "}
												{model.tileSize}{" "}
												{LocalizeText(
													"navigator.createroom.tilesize"
												)}
											</Text>
											{!hcDisabled &&
												model.clubLevel >
													HabboClubLevelEnum.NO_CLUB && (
													<Base className="icon icon-hc_mini position-absolute top-1 end-1" />
												)}
											{selectedModelName && (
												<i className="active-arrow" />
											)}
										</RoomCreatorGridItem>
									);
								})}
