...
export const FriendBarItemView: FC<{ friend: MessengerFriend }> = (props) => {
	...
	if (!friend) {
		return (
			<div
				ref={elementRef}
				className={
					"btn btn-friendsgen friend-bar-item friend-bar-search " +
					(isVisible ? "friend-bar-search-item-active" : "")
				}
				onClick={(event) => setVisible((prevValue) => !prevValue)}
			>
				<div className="friend-bar-item-head position-absolute" />
				<div className="friend-bar-text">
				...
				{isVisible && (
					<div className="search-content mt-3">
						...
						<Button
							...
							onClick={() =>
								SendMessageComposer(
									new FindNewFriendsMessageComposer()
								)
							}
						>
	...
	return (
		<div
			ref={elementRef}
			className={
				"btn btn-friendsgensuccess friend-bar-item " +
				(isVisible ? "friend-bar-item-active" : "")
			}
			onClick={(event) => setVisible((prevValue) => !prevValue)}
		>
			<div
