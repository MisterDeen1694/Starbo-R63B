...
export const FriendsListView: FC<{}> = (props) => {
	const [isFromSearchToolbar, setIsFromSearchToolbar] =
	...
	const [showRemoveFriendsConfirmation, setShowRemoveFriendsConfirmation] =
		useState<boolean>(false);
	const {
		onlineFriends = [],
		offlineFriends = [],
	...
	const removeFriendsText = useMemo(() => {
		...
		for (const userId of selectedFriendsIds) {
			let existingFriend: MessengerFriend = onlineFriends.find(
				(f) => f.id === userId
			);

			if (!existingFriend)
				existingFriend = offlineFriends.find((f) => f.id === userId);
		...
		return LocalizeText(
			"friendlist.removefriendconfirm.userlist",
			["user_names"],
			[userNames.join(", ")]
		);
	...
	const sendRoomInvite = (message: string) => {
		...
		SendMessageComposer(
			new SendRoomInviteComposer(message, selectedFriendsIds)
		);
				...
				>
					<NitroCardAccordionView fullHeight overflow="hidden">
						...
						<NitroCardAccordionSetView
							headerText={
								LocalizeText(
									"friendlist.friends.offlinecaption"
						...
						/>
						<FriendsSearchView
							headerText={LocalizeText("people.search.title")}
						/>
					</NitroCardAccordionView>
					{selectedFriendsIds && selectedFriendsIds.length > 0 && (
						<Flex gap={1} className="p-1">
							<Button onClick={() => setShowRoomInvite(true)}>
								{LocalizeText("friendlist.tip.invite")}
							</Button>
							<Button
								variant="primary"
								onClick={(event) =>
									setShowRemoveFriendsConfirmation(true)
								}
							>
