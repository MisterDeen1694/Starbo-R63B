...
export const FriendsRoomInviteView: FC<FriendsRoomInviteViewProps> = (
	props
) => {
	const {
	...
	return (
		...
		>
			<NitroCardHeaderView
				headerText={LocalizeText("friendlist.invite.title")}
				onCloseClick={onCloseClick}
			/>
			<NitroCardContentView className="text-black">
				{LocalizeText(
					"friendlist.invite.summary",
					["count"],
					[selectedFriendsIds.length.toString()]
				)}
				<textarea
					...
					maxLength={255}
					onChange={(event) =>
						setRoomInviteMessage(event.target.value)
					}
				></textarea>
				...
				<Flex gap={1}>
					<Button
						variant="primary"
						disabled={
							roomInviteMessage.length === 0 ||
							selectedFriendsIds.length === 0
						}
