...
export const ChooserWidgetView: FC<ChooserWidgetViewProps> = (props) => {
	...
	const ownerNames = useMemo(() => {
		const names = Array.from(
			new Set(items.map((item) => item.ownerName || "Unknown"))
		);
		return names.sort();
	}, [items]);
	...
	return (
		<NitroCardView className="nitro-chooser-widget" theme="primary-slim">
			<NitroCardHeaderView
				headerText={title + " (" + filteredItems.length + ")"}
				...
				<Flex gap={2}>
					...
					{pickallFurni && (
						...
						>
							<option value="all">
								{LocalizeText(
									"roomsettings.access_rights.anyone"
								)}
							</option>
							{ownerNames.length > 0 ? (
				...
				{pickallFurni && (
					<Flex gap={2} className="text-black">
						...
						<label className="form-check-label">
							{LocalizeText("widget.chooser.checkall")}
						</label>
					</Flex>
				)}
				<InfiniteScroll
