...
export const RoomFilterWordsWidgetView: FC<{}> = (props) => {
	...
	const processAction = (isAddingWord: boolean) => {
		...
		if (
			isAddingWord &&
			wordsFilter.includes(isSelectingWord ? selectedWord : word)
		)
			return;
		...
		setWordsFilter((prevValue) => {
			...
			isAddingWord
				...
				: newWords.splice(
						newWords.indexOf(isSelectingWord ? selectedWord : word),
						1
				  );
			...
			<NitroCardContentView className="text-black">
				<Grid className="d-flex align-items-center justify-content-end gap-2">
					...
					<Button onClick={() => processAction(true)}>
						{LocalizeText(
							"navigator.roomsettings.roomfilter.addword"
						)}
					</Button>
				</Grid>
				<Column
				...
				<Grid className="d-flex align-items-center justify-content-end gap-2">
					...
					>
						{LocalizeText(
							"navigator.roomsettings.roomfilter.removeword"
						)}
					</Button>
				</Grid>
			</NitroCardContentView>
