							...
							return (
								<Flex
									...
									className={
										...
										(selectedItem === index
											? "border border-muted border-2 rounded"
											: "border-2")
									}
									alignItems="center"
									onClick={() =>
			...
			{(!playlist || playlist.length === 0) && (
				<>
					<div className="playlist-bottom text-black p-1 ms-5">
						<h5>
							{LocalizeText(
								"playlist.editor.add.songs.to.your.playlist"
							)}
						</h5>
						<div>
							{LocalizeText(
								"playlist.editor.text.click.song.to.choose.click.again.to.move"
							)}
						</div>
					</div>
					<img
						src={
							GetConfiguration("image.library.url") +
							"playlist/background_add_songs.gif"
						}
						className="add-songs"
					/>
				</>
			...
			{playlist && playlist.length > 0 && (
				<>
					{currentPlayingIndex === -1 && (
						<Button
							variant="primary"
							onClick={() => playPause(furniId, selectedItem)}
						>
						...
						<Flex gap={1}>
							...
							<Flex column>
								<Text bold display="block">
									{LocalizeText(
										"playlist.editor.text.now.playing.in.your.room"
									)}
								</Text>
								<Text>
