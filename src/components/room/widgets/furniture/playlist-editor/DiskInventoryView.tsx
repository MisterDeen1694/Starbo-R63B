...
export const DiskInventoryView: FC<DiskInventoryViewProps> = (props) => {
	...
	useEffect(() => {
		...
		GetNitroInstance().soundManager.musicController?.playSong(
			previewSongId,
			MusicPriorities.PRIORITY_SONG_PLAY,
			0,
			0,
			0,
			0
		...
		return () => {
			GetNitroInstance().soundManager.musicController?.stop(
				MusicPriorities.PRIORITY_SONG_PLAY
			);
		};
	}, [previewSongId]);
	...
	return (
		<>
			...
			<div className="playlist-bottom text-black p-1">
				...
				<div>
					{LocalizeText(
						"playlist.editor.text.you.have.no.songdisks.available"
					)}
				</div>
				<div>
					{LocalizeText(
						"playlist.editor.text.you.can.buy.some.from.the.catalogue"
					)}
				</div>
				<button
			...
			<img
				src={
					GetConfiguration("image.library.url") +
					"playlist/background_get_more_music.gif"
				}
				className="get-more"
			/>
		</>
