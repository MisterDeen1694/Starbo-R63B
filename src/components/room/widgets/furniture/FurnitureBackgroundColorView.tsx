...
export const FurnitureBackgroundColorView: FC<{}> = (props) => {
	...
	return (
		<NitroCardView theme="primary-slim" className="nitro-room-widget-toner">
			<NitroCardHeaderView
				headerText={LocalizeText("widget.backgroundcolor.title")}
			...
			<NitroCardContentView overflow="hidden" justifyContent="between">
				<Column overflow="auto" gap={1}>
					<input
						...
						onChange={(event) =>
							setColor(
								ColorUtils.convertFromHex(event.target.value)
							)
						}
					/>
				</Column>
				<Column gap={1}>
					<Button variant="primary" onClick={toggleToner}>
						{LocalizeText("widget.backgroundcolor.button.on")}
					</Button>
					<Button variant="primary" onClick={applyToner}>
