			...
			>
				<NitroCardHeaderView
					headerText={LocalizeText(
						"friend.furniture.confirm.lock.caption"
					)}
				...
				<NitroCardContentView>
					...
					{stage === 2 && (
						<div className="text-small text-black text-center mb-2">
							{LocalizeText(
								"friend.furniture.confirm.lock.other.locked"
							)}
						</div>
					)}
					<Flex gap={1}>
						<Button onClick={(event) => respond(false)}>
							{LocalizeText(
								"friend.furniture.confirm.lock.button.cancel"
							)}
						</Button>
						<Button
							variant="primary"
							onClick={(event) => respond(true)}
						>
							{LocalizeText(
								"friend.furniture.confirm.lock.button.confirm"
							)}
						</Button>
					</Flex>
				</NitroCardContentView>
	...
	if (usernames.length > 0) {
		return (
			<DraggableWindow handleSelector=".nitro-engraving-lock-view">
				<div
					className={`nitro-engraving-lock-view engraving-lock-${type}`}
				>
					...
					<Column justifyContent="between" className="mt-1">
						...
						>
							<div>
								...
								{type === 3 &&
									LocalizeText("wildwest.engraving.caption")}
							</div>
							<div>{date}</div>
						</Column>
