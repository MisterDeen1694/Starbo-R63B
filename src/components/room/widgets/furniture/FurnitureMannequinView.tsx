...
export const FurnitureMannequinView: FC<{}> = (props) => {
	...
	useEffect(() => {
		...
		if (
			GetSessionDataManager().gender.toLowerCase() !==
			gender.toLowerCase()
		) {
	...
	useEffect(() => {
		switch (mode) {
			...
			case MODE_UPDATE: {
				const figureContainer =
					GetAvatarRenderManager().createFigureContainer(
			...
			case MODE_NO_CLUB: {
				const figureContainer =
					MannequinUtilities.getMergedMannequinFigureContainer(
						GetSessionDataManager().figure,
						figure
					);
	...
	return (
		...
		>
			<NitroCardHeaderView
				headerText={LocalizeText("mannequin.widget.title")}
					...
					<Column grow justifyContent="between" overflow="auto">
						...
						{mode === MODE_PEER && (
							<>
								<Column gap={1}>
									...
									<Text>
										{LocalizeText(
											"mannequin.widget.weartext"
										)}
									</Text>
								</Column>
								<Button variant="primary" onClick={wearFigure}>
									{LocalizeText("mannequin.widget.wear")}
								</Button>
							</>
						)}
						{mode === MODE_NO_CLUB && (
							<Flex center grow>
								<Text>
									{LocalizeText(
										"mannequin.widget.clubnotification"
									)}
								</Text>
							</Flex>
						)}
