...
export const FurnitureGiftOpeningView: FC<{}> = (props) => {
	...
	const place = (itemId: number) => {
		const groupItem = groupItems.find(
			(group) => group.getItemById(itemId)?.id === itemId
		);
		...
		<NitroCardView className="nitro-gift-opening" theme="primary-slim">
			<NitroCardHeaderView
				headerText={LocalizeText(
					senderName
						? "widget.furni.present.window.title_from"
						: "widget.furni.present.window.title",
					["name"],
					[senderName]
				)}
						...
						{isOwnerOfFurniture && (
							<Flex gap={1}>
								{senderName && (
									...
									>
										{LocalizeText(
											"widget.furni.present.give_gift",
											["name"],
											[senderName]
										)}
									</Button>
								)}
								<Button variant="primary" onClick={openPresent}>
									{LocalizeText(
										"widget.furni.present.open_gift"
									)}
								</Button>
							</Flex>
						)}
							...
							<Column grow gap={1}>
								<Flex gap={1}>
									{placedInRoom && (
										...
										>
											{LocalizeText(
												"widget.furni.present.put_in_inventory"
											)}
										</Button>
									)}
									<Button
										variant="primary"
										onClick={(event) => place(placedItemId)}
									>
										{LocalizeText(
											placedInRoom
