...
export const PurchasableClothingConfirmView: FC<
	PurchasableClothingConfirmViewProps
> = (props) => {
	...
	useEffect(() => {
				...
				switch (furniData.specialType) {
					case FurniCategory.FIGURE_PURCHASABLE_SET:
						...
						for (const setId of setIds) {
							if (
								GetAvatarRenderManager().isValidFigureSetForGender(
									setId,
									gender
								)
							)
		...
		setGender(gender);
		setNewFigure(
			GetAvatarRenderManager().getFigureStringWithFigureIds(
				figure,
				gender,
				validSets
			)
				...
				<Flex gap={2} overflow="hidden">
					...
					<Column justifyContent="between" overflow="auto">
						<Column gap={2}>
							...
							<Text>
								{LocalizeText(
									"useproduct.widget.info.bind_clothing"
								)}
							</Text>
						</Column>
						<Flex alignItems="center" justifyContent="between">
							...
							<Button variant="primary" onClick={useProduct}>
								{LocalizeText(
									"useproduct.widget.bind_clothing"
								)}
							</Button>
						</Flex>
					</Column>
