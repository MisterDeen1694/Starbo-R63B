...
export const FurnitureDimmerView: FC<{}> = (props) => {
	...
	const onClose = () => {
		FurnitureDimmerUtilities.previewDimmer(
			color,
			brightness,
			effectId === 2
		);
	...
	const isFreeColorMode = useMemo(
		() => GetConfiguration<boolean>("widget.dimmer.colorwheel", false),
		[]
	);
	...
	return (
		<NitroCardView className="nitro-room-widget-dimmer">
			<NitroCardHeaderView
				headerText={LocalizeText("widget.dimmer.title")}
				...
				{dimmerState === 0 && (
					<Column alignItems="center">
						...
						<Button
							variant="primary"
							onClick={() =>
								FurnitureDimmerUtilities.changeState()
							}
						>
				...
				{dimmerState === 1 && (
					<>
						<Column gap={1}>
							...
							{isFreeColorMode && (
								<input
									...
									value={ColorUtils.makeColorNumberHex(
										selectedColor
									)}
									onChange={(event) =>
										setSelectedColor(
											ColorUtils.convertFromHex(
												event.target.value
											)
										)
									}
							...
							{!isFreeColorMode && (
								<Grid gap={1} columnCount={7}>
									{FurnitureDimmerUtilities.AVAILABLE_COLORS.map(
										(color, index) => {
											return (
												<Column
													fullWidth
						...
						<Column gap={1}>
							<Text fontWeight="bold">
								{LocalizeText(
									"widget.backgroundcolor.lightness"
								)}
							</Text>
							<Slider
								...
								renderThumb={(props, state) => (
									<div {...props}>
										{FurnitureDimmerUtilities.scaleBrightness(
											state.valueNow
										)}
									</div>
								)}
						...
						<Flex alignItems="center" gap={1}>
							...
							/>
							<Text>
								{LocalizeText("widget.dimmer.type.checkbox")}
							</Text>
						</Flex>
						<Flex gap={1}>
							<Button
