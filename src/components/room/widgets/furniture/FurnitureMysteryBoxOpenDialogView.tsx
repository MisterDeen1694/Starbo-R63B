...
export const FurnitureMysteryBoxOpenDialogView: FC<
	FurnitureMysteryBoxOpenDialogViewProps
> = (props) => {
	...
	const close = () => {
		if (mode === ViewMode.WAITING)
			SendMessageComposer(
				new MysteryBoxWaitingCanceledMessageComposer(ownerId)
			);
		setMode(ViewMode.HIDDEN);
		setPrizeData(undefined);
	};

	useMessageEvent<ShowMysteryBoxWaitMessageEvent>(
		ShowMysteryBoxWaitMessageEvent,
		(event) => {
	...
	useMessageEvent<CancelMysteryBoxWaitMessageEvent>(
		CancelMysteryBoxWaitMessageEvent,
		(event) => {
			setMode(ViewMode.HIDDEN);
			setPrizeData(undefined);
	...
	useMessageEvent<GotMysteryBoxPrizeMessageEvent>(
		GotMysteryBoxPrizeMessageEvent,
		(event) => {
			const parser = event.getParser();
			setPrizeData({
				contentType: parser.contentType,
				classId: parser.classId,
			});
			...
			<NitroCardContentView>
				...
				{mode === ViewMode.PRIZE && prizeData && (
					<>
						<Text variant="black">
							{" "}
							{LocalizeText("mysterybox.reward.text")}{" "}
						</Text>
