...
export const AvatarInfoUseProductConfirmView: FC<
	AvatarInfoUseProductConfirmViewProps
> = (props) => {
	...
	const getPetImage = useMemo(() => {
		...
		switch (furniData.specialType) {
			case FurniCategory.PET_SHAMPOO: {
				...
				const currentPalette = GetRoomEngine().getPetColorResult(
					petIndex,
					petFigureData.paletteId
				);
				const possiblePalettes =
					GetRoomEngine().getPetColorResultsForTag(
						petIndex,
						customParts[1]
					);
			...
			case FurniCategory.PET_CUSTOM_PART: {
				...
				const _local_6 = customParts[1]
					.split(",")
					.map((piece) => parseInt(piece));
				const _local_7 = customParts[2]
					.split(",")
					.map((piece) => parseInt(piece));
				const _local_8 = customParts[3]
					.split(",")
					.map((piece) => parseInt(piece));
			...
			case FurniCategory.MONSTERPLANT_FERTILIZE: {
				...
				if (roomObject) {
					posture = roomObject.model.getValue<string>(
				...
				<Flex gap={2} overflow="hidden">
					...
					<Column justifyContent="between" overflow="auto">
						<Column gap={2}>
							{mode === PRODUCT_PAGE_SHAMPOO && (
								<>
									...
									<Text>
										{LocalizeText(
											"useproduct.widget.info.shampoo"
										)}
									</Text>
								</>
							)}
							{mode === PRODUCT_PAGE_CUSTOM_PART && (
								<>
									...
									<Text>
										{LocalizeText(
											"useproduct.widget.info.custompart"
										)}
									</Text>
								</>
							)}
							{mode === PRODUCT_PAGE_CUSTOM_PART_SHAMPOO && (
								<>
									...
									<Text>
										{LocalizeText(
											"useproduct.widget.info.custompartshampoo"
										)}
									</Text>
								</>
							)}
							...
							{mode === PRODUCT_PAGE_REVIVE && (
								<>
									...
									<Text>
										{LocalizeText(
											"useproduct.widget.info.revive_monsterplant"
										)}
									</Text>
								</>
							)}
							{mode === PRODUCT_PAGE_REBREED && (
								<>
									...
									<Text>
										{LocalizeText(
											"useproduct.widget.info.rebreed_monsterplant"
										)}
									</Text>
								</>
							)}
							{mode === PRODUCT_PAGE_FERTILIZE && (
								<>
									...
									<Text>
										{LocalizeText(
											"useproduct.widget.info.fertilize_monsterplant"
										)}
									</Text>
								</>
							)}
						...
						<Flex alignItems="center" justifyContent="between">
							...
							<Button variant="primary" onClick={useProduct}>
