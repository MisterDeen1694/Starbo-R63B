...
export const InfoStandWidgetFurniView: FC<InfoStandWidgetFurniViewProps> = (
	...
	const [dropdownOpen, setDropdownOpen] = useState(
		sessionStorage.getItem("dropdownOpen") === "true"
	);
	...
	function getRotationIndex(directionVector) {
		...
		switch (angle) {
			...
			case 315:
				return 7;
			default:
				return null; // Handle unexpected angles
		}
	}
	...
	useEffect(() => {
		...
		const locationString = GetRoomEngine().getFurniLocation(
			roomId,
			objectId,
			isWallItem
		);
		...
		const directionString = GetRoomEngine().getFurniDirection(
			roomId,
			objectId,
			isWallItem
		);
		const directionVector = parseVector3d(directionString);
		const rotationIndex = directionVector
			? getRotationIndex(directionVector)
			: null;

		const state = GetRoomEngine().getFurniState(
			roomId,
			objectId,
			isWallItem
		);
	...
	function parseVector3d(vectorString: string) {
		...
		const matches = vectorString.match(
			/\[Vector3d: ([\d.]+), ([\d.]+), ([\d.]+)/
		);
	...
	useMessageEvent<FurnitureFloorUpdateEvent>(
		FurnitureFloorUpdateEvent,
		(event) => {
	...
	function getValidRoomObjectDirection(roomObject, isPositive) {
		...
		if (roomObject.type === "monster_plant") {
			allowedDirections = roomObject.model.getValue(
				"pet_allowed_directions"
			);
		} else {
			allowedDirections = roomObject.model.getValue(
	...
	useSoundEvent<NowPlayingEvent>(
		...
		(event) => {
			...
			const songInfo =
				GetNitroInstance().soundManager.musicController.getSongInfo(
					event.id
				);
	...
	useEffect(() => {
		...
		if (
			avatarInfo.usagePolicy ===
				RoomWidgetFurniInfoUsagePolicyEnum.EVERYBODY ||
			(avatarInfo.usagePolicy ===
	...
	useMessageEvent<GroupInformationEvent>(GroupInformationEvent, (event) => {
		...
		if (!avatarInfo || avatarInfo.groupId !== parser.id || parser.flag)
			return;
	...
	const processButtonAction = useCallback(
		(action: string) => {
			...
			switch (action) {
				case "buy_one":
					CreateLinkEvent(
						`catalog/open/offerId/${avatarInfo.purchaseOfferId}`
					);
					return;
				case "move":
		...
		<Column gap={1} alignItems="end">
			<Column className="nitro-infostand">
				...
				>
						...
						<Flex position="relative" gap={1}>
							{avatarInfo.stuffData.isUnique && (
								<div className="position-absolute end-0">
									<LayoutLimitedEditionCompactPlateView
										uniqueNumber={
											avatarInfo.stuffData.uniqueNumber
										}
										uniqueSeries={
											avatarInfo.stuffData.uniqueSeries
										}
									/>
								</div>
							)}
					...
					<Column gap={1}>
						<Text fullWidth wrap textBreak variant="white">
						...
						<Column gap={1}>
							...
							{songId === -1 && (
								<Text variant="white" small wrap>
									{LocalizeText(
										"infostand.jukebox.text.not.playing"
									)}
								</Text>
							)}
							{!!songName.length && (
							...
							<>
								...
								<Text variant="white" wrap>
									{LocalizeText(
										...
										["hits", "target"],
										[
											crackableHits.toString(),
											crackableTarget.toString(),
										]
									)}
								</Text>
							</>
								...
								{!avatarInfo.isWallItem && canMove && (
									<>
										...
										>
											{dropdownOpen
												? `${LocalizeText(
														"widget.furni.present.close"
												  )} Buildtools`
												: `${LocalizeText(
														"navigator.roomsettings.doormode.open"
												  )} Buildtools`}
										</Button>
										{dropdownOpen && (
											<>
														...
														<Column fullWidth>
															...
															>
																<Flex className="floor-spaceing">
																	<Base
																		className="buildtool-movefurni button-leftup"
																		onClick={() =>
																			sendUpdate(
																				-1,
																				0,
																	...
																	>
																		{" "}
																		<GrFormNextLink
																			className="fa-icon icon-color"
																			size="1.7em"
																		/>{" "}
																	</Base>
																	<Base
																		className="buildtool-movefurni button-rightup"
																		onClick={() =>
																			sendUpdate(
																				0,
																				-1,
																...
																<Flex className="floor-spaceing">
																	<Base
																		className="buildtool-movefurni button-leftdown"
																		onClick={() =>
																			sendUpdate(
																				0,
																				1,
																	...
																	<Base
																		className="buildtool-movefurni button-rightdown"
																		onClick={() =>
																			sendUpdate(
																				1,
																				0,
															...
															<Text variant="white">
																{LocalizeText(
																	"infostand.button.rotate"
																)}
															</Text>
															<Flex
																center
																className="floor-spaceing"
															>
																<Base
																	className="buildtool-rotatefurni"
																	onClick={() =>
																		sendUpdate(
																			0,
																			0,
													...
													<Column className="buildtool-box buildtool-box-right">
														<Column fullWidth>
															...
															>
																<Column>
																	<Base
																		className="buildtool-setheight buildtool-setheightup"
																		onClick={() =>
																			adjustHeight(
																				1
																			)
																	...
																	<Base
																		className="buildtool-setheight buildtool-setheightdown"
																		onClick={() =>
																			adjustHeight(
																				-1
																			)
			...
			<Flex gap={2} justifyContent="end">
				{canMove && (
					<Button onClick={(event) => processButtonAction("move")}>
						{LocalizeText("infostand.button.move")}
					</Button>
				)}
				{canRotate && (
					<Button onClick={(event) => processButtonAction("rotate")}>
						{LocalizeText("infostand.button.rotate")}
					</Button>
				)}
				{pickupMode !== PICKUP_MODE_NONE && (
					<Button onClick={(event) => processButtonAction("pickup")}>
						{LocalizeText(
							pickupMode === PICKUP_MODE_EJECT
				...
				{canUse && (
					<Button onClick={(event) => processButtonAction("use")}>
