...
export const HcCenterView: FC<{}> = (props) => {
	...
	const getClubText = () => {
		...
		if (
			purse.minutesUntilExpiration > -1 &&
			purse.minutesUntilExpiration < 60 * 24
		) {
			return FriendlyTime.shortFormat(purse.minutesUntilExpiration * 60);
		}

		return FriendlyTime.shortFormat(
			(purse.clubPeriods * 31 + purse.clubDays) * 86400
		);
	};
			...
			case ClubStatus.ACTIVE:
				return LocalizeText(
					...
					[
						...
						FriendlyTime.shortFormat(
							kickbackData?.currentHcStreak * 86400
						),
					]
				);
		...
		LocalizeText(
			...
			[
				(
					...
					kickbackData?.creditRewardForMonthlySpent
				).toString(),
			]
		);
	...
	useMessageEvent<ScrSendKickbackInfoMessageEvent>(
		ScrSendKickbackInfoMessageEvent,
		(event) => {
			const parser = event.getParser();
				...
				<div>
					{LocalizeText(
						...
						[
							getHcPaydayAmount(),
							(
								((kickbackData?.kickbackPercentage *
									kickbackData?.totalCreditsSpent +
									kickbackData?.creditRewardForStreakBonus) *
									100) /
								100
							).toString(),
					...
					<Flex>
						<Button
							...
							onClick={(event) =>
								CreateLinkEvent(
									"catalog/open/" +
										GetConfiguration("catalog.links")[
											"hc.buy_hc"
										]
								)
							}
				...
				{GetConfiguration("hc.center")["payday.info"] && (
					<Flex alignItems="center">
						...
						<div className="payday flex-shrink-0 p-2">
							...
							{clubStatus === ClubStatus.ACTIVE && (
								<div className="pe-3">
									<h5 className="ms-2 mb-1 bolder">
										{LocalizeText(
											"hccenter.special.amount.title"
										)}
									</h5>
									<div className="d-flex flex-column">
										<div className="w-100 text-center ms-4n">
											{getHcPaydayAmount()}
										</div>
										<OverlayTrigger
											trigger={["hover", "focus"]}
