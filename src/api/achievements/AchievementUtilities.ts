...
export class AchievementUtilities {
	public static getAchievementBadgeCode(
		...
		if (!achievement.finalLevel)
			badgeId = GetLocalization().getPreviousLevelBadgeId(badgeId);
	...
	public static getAchievementCategoryImageUrl(
		...
		let imageName = icon
			? "categories/ach_icon_"
			: "categories/ach_category_";

		imageName += category.code;

		// if (progress !== null)
		// 	imageName += `_${progress > 0 ? "active" : "inactive"}`;

		return imageUrl.replace("%image%", imageName);
	}

	public static getAchievementCategoryMaxProgress(
		category: IAchievementCategory
	): number {
	...
	public static getAchievementCategoryProgress(
		category: IAchievementCategory
	): number {
		if (!category) return 0;
		...
		for (const achievement of category.achievements)
			progress += achievement.finalLevel
				? achievement.level
				: achievement.level - 1;
	...
	public static getAchievementCategoryTotalUnseen(
		category: IAchievementCategory
	): number {
		if (!category) return 0;
		...
		for (const achievement of category.achievements)
			achievement.unseen > 0 && unseen++;

		return unseen;
	}

	public static getAchievementHasStarted(
		achievement: AchievementData
	): boolean {
	...
	public static getAchievementIsIgnored(
		achievement: AchievementData
	): boolean {
		if (!achievement) return false;

		const ignored = GetConfiguration<string[]>(
